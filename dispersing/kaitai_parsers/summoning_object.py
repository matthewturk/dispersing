...
class SummoningObject(KaitaiStruct):
    ...
    class ObjectRecord(KaitaiStruct):
        SEQ_FIELDS = ["ac_bonus", "col0", "object_name_id", "weight", "container_flags", "col4", "middle_icon", "act1_ndice", "act1_nsides", "act2_ndice", "act2_nsides", "act3_ndice", "act3_nsides", "charges", "image_id", "col11", "subroutine", "obj_type", "scroll_id"]
        def __init__(self, _io, _parent=None, _root=None):
            self._io = _io
            self._parent = _parent
            self._root = _root if _root else self
        ...
        def _read(self):
            ...
            self._debug['middle_icon']['start'] = self._io.pos()
            self.middle_icon = self._io.read_u1()
            self._debug['middle_icon']['end'] = self._io.pos()
            self._debug['act1_ndice']['start'] = self._io.pos()
            self.act1_ndice = self._io.read_bits_int_be(4)
            self._debug['act1_ndice']['end'] = self._io.pos()
            self._debug['act1_nsides']['start'] = self._io.pos()
            self.act1_nsides = self._io.read_bits_int_be(4)
            self._debug['act1_nsides']['end'] = self._io.pos()
            self._debug['act2_ndice']['start'] = self._io.pos()
            self.act2_ndice = self._io.read_bits_int_be(4)
            self._debug['act2_ndice']['end'] = self._io.pos()
            self._debug['act2_nsides']['start'] = self._io.pos()
            self.act2_nsides = self._io.read_bits_int_be(4)
            self._debug['act2_nsides']['end'] = self._io.pos()
            self._debug['act3_ndice']['start'] = self._io.pos()
            self.act3_ndice = self._io.read_bits_int_be(4)
            self._debug['act3_ndice']['end'] = self._io.pos()
            self._debug['act3_nsides']['start'] = self._io.pos()
            self.act3_nsides = self._io.read_bits_int_be(4)
            self._debug['act3_nsides']['end'] = self._io.pos()
            self._io.align_to_byte()
            ...
            self._debug['subroutine']['start'] = self._io.pos()
            self.subroutine = self._io.read_u1()
            self._debug['subroutine']['end'] = self._io.pos()
