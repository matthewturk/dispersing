...
class SummoningNpc(KaitaiStruct):
    ...
    class MagicAttackTypes(IntEnum):
        none = 0
        stone = 1
        poison = 3
        fire = 4
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(SummoningNpc, self).__init__(_io)
    ...
    def _read(self):
        ...
        for i in range(self.count):
            ...
            self.npcs.append(SummoningNpc.Npc(self._io, self, self._root))
    ...
    def _fetch_instances(self):
        ...
        for i in range(len(self.npcs)):
            pass
            self.npcs[i]._fetch_instances()


    class Npc(KaitaiStruct):
        SEQ_FIELDS = ["npc_id", "head_id", "flags", "maybe_n_hit_dice", "damage_resistance", "damage_bonus", "conditionally_hostile", "sprite_id", "col8", "action", "col10", "col11", "magic_attack", "weapon_vulnerabilities", "col14", "behavior_flags"]
        def __init__(self, _io, _parent=None, _root=None):
            super(SummoningNpc.Npc, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._debug = collections.defaultdict(dict)
            self._read()
