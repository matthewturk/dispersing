...
class SummoningInit(KaitaiStruct):
    SEQ_FIELDS = ["sprite_offsets", "unknown1", "unknown2", "unknown3", "unknown4", "unknown5", "spell_cost", "spell_dice", "spells", "unknown8", "unknown9", "unknown10", "unknown11"]
    ...
    def _read(self):
        ...
        self._debug['spell_dice']['start'] = self._io.pos()
        self.spell_dice = [None] * (40)
        for i in range(40):
            if not 'arr' in self._debug['spell_dice']:
                self._debug['spell_dice']['arr'] = []
            self._debug['spell_dice']['arr'].append({'start': self._io.pos()})
            self.spell_dice[i] = SummoningInit.SpellInfo(self._io, self, self._root)
            self._debug['spell_dice']['arr'][i]['end'] = self._io.pos()

        self._debug['spell_dice']['end'] = self._io.pos()
    ...
    class SpellInfo(KaitaiStruct):
        SEQ_FIELDS = ["num_dice", "dice_faces", "offset_dice_faces"]
        def __init__(self, _io, _parent=None, _root=None):
            self._io = _io
            self._parent = _parent
            self._root = _root if _root else self
        ...
        def _read(self):
            self._debug['num_dice']['start'] = self._io.pos()
            self.num_dice = self._io.read_u1()
            self._debug['num_dice']['end'] = self._io.pos()
            self._debug['dice_faces']['start'] = self._io.pos()
            self.dice_faces = self._io.read_u1()
            self._debug['dice_faces']['end'] = self._io.pos()
            self._debug['offset_dice_faces']['start'] = self._io.pos()
            self.offset_dice_faces = self._io.read_u1()
            self._debug['offset_dice_faces']['end'] = self._io.pos()
