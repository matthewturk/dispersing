...
class LevelMap:
    ...
    def to_cp437(self, overrides=None):
        ...
        new_string_components = [
            b"".join(
                [b" "]
                + [str(i)[-1].encode("cp437") for i in range(self.tiles.shape[1])]
            )
        ]
        for i, row in enumerate(self.tiles):
            cols = [str(i)[-1].encode("cp437")]
            for col in row:
                cols.append(tile_map[col])
            new_string_components.append(b"".join(cols))
        new_string = b"\n".join(new_string_components)
